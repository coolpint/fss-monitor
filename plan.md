...
- 도이치은행 제재관련 공시 누락 제보를 확인했다.
- GitHub Actions 로그 확인 결과, 도이치은행 공시는 2026-04-30 03:11 KST 실행에서 Teams 링크 알림 발송 완료 및 `seen.json` 저장까지 완료됐다.
- 다만 도이치은행 처리로 `latest_notice_date`가 2026.04.27로 올라간 뒤, 2026.04.24 경남은행/부산은행/국민은행 및 2026.04.15 아이케이파트너스 공시가 "과거 일자"로 판단되어 알림 없이 `seen.json`에 기록된 버그를 확인했다.
- 해결: `seen.json`에 없는 공시는 공시일이 최신 기준일보다 과거여도 알림하도록 `monitor.py`의 과거 일자 스킵 로직을 제거한다.
- 복구: 알림 없이 `seen.json`에 기록된 4건은 상태 파일에서 제거해 다음 정상 실행에서 다시 알림되도록 한다.
- 회귀 방지: 과거 일자 신규 공시도 알림되는지, 알림 실패 시 `seen.json`에 기록하지 않는지 단위 테스트를 추가한다.
- 후속 조치: 도이치은행 공시는 로컬 기사화 작업 패키지를 생성해 `runs/20260427_202500015_2_도이치은행-제재관련-공시/`에 `gemini_ready` 상태로 준비했다.
...
- 날짜 high-water mark는 상태 참고값으로만 유지하고, 알림 여부는 `seen_keys`를 기준으로 판단하도록 변경한다.
- 금감원 목록에 늦게 나타난 과거 공시를 알림 없이 처리済로 만드는 동작을 제거했다.
- 2026-04-30 실행에서 알림 없이 처리済가 된 4건은 `seen.json`에서 제거해 재알림 가능 상태로 되돌렸다.
- 도이치은행 공시 PDF 다운로드와 텍스트 추출을 완료해 Gemini 기사 입력 파일과 CMS 인계 템플릿을 생성했다.
